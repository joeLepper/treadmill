"""Create validator_gold_rows table (ADR-0070 §3 candidate-content layer).

Establishes the ``validator_gold_rows`` Postgres table with the ADR-0070
six-layer shape:

  1. Provenance (id, created_at, source_* nullable fields)
  2. Candidate content (source_step_id, verdict_emitted, script_excerpt,
     artifact_excerpt)
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("artifact_excerpt", sa.Text(), nullable=False),
        # ── LLM recommendation ────────────────────────────────────────────────
        ...
        sa.CheckConstraint(
            ...
            name="ck_validator_gold_rows_verdict_emitted",
        ),
        sa.CheckConstraint(
            "llm_label IN ('correct-verdict', 'wrong-verdict', 'unclear')",
            name="ck_validator_gold_rows_llm_label",
        ),
        sa.CheckConstraint(
            "llm_confidence IN ('high', 'medium', 'low')",
            name="ck_validator_gold_rows_llm_confidence",
        ),
        sa.CheckConstraint(
            "label_verdict IS NULL OR label_verdict IN "
            "('correct-verdict', 'wrong-verdict', 'unclear')",
            name="ck_validator_gold_rows_label_verdict",
        ),
        sa.CheckConstraint(
            "outcome_state IS NULL OR outcome_state IN "
            "('pending', 'merged', 'rejected', 'superseded', 'cancelled')",
            name="ck_validator_gold_rows_outcome_state",
        ),
    )

    # ── Plain indexes ─────────────────────────────────────────────────────────
    op.create_index(
        "ix_validator_gold_rows_created_at",
        "validator_gold_rows",
        ["created_at"],
    )
    op.create_index(
        "ix_validator_gold_rows_verdict_emitted",
        "validator_gold_rows",
        ["verdict_emitted"],
    )
    ...
    op.create_index(
        "ix_validator_gold_rows_unlabeled",
        "validator_gold_rows",
        ["label_verdict"],
...
def downgrade() -> None:
    op.drop_index(
        "ix_validator_gold_rows_unlabeled", table_name="validator_gold_rows"
    )
    op.drop_index(
        "ix_validator_gold_rows_verdict_emitted", table_name="validator_gold_rows"
    )
    op.drop_index(
        "ix_validator_gold_rows_created_at", table_name="validator_gold_rows"
    )
    op.drop_table("validator_gold_rows")
