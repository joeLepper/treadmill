...
_ROLES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "system_prompt": (
            ...
            "the reviewer was wrong, the work is fine — the system "
            "emits a synthetic ``review.override`` event that unblocks "
            "auto-merge). **For deadlock-trigger runs, only use "
            "``accept-as-is`` when you have specifically compared the "
            "diff against the task spec and confirmed every spec item "
            "is present in the diff. Reviewer approval alone is "
            "insufficient — the validator is reading the spec, the "
            "reviewer is reading the diff in isolation.** Note: today's "
            ...
            "**Bias toward amend for deadlock with validate-fail.** "
            "If the validator cited specific missing pieces (files not "
            "in the diff, symbols not wired, spans not emitted), and "
