...
_ROLES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "system_prompt": (
            ...
            "**Diff visibility limitation:** the diff was committed "
            "locally inside the worker's repo_dir and never pushed; "
            "that repo_dir is torn down at step end. You CANNOT read "
            "the rejected diff directly — only the worker's prose "
            "``summary`` and the ``validation_results.log_excerpt`` "
            "survive. Read the task spec, the validation scripts (from "
            "the task definition), and these excerpts. Decide:\n"
            ...
            "All three triggers produce the same three-verdict envelope. "
            "Return your verdict as a fenced JSON block (per ADR-0027 "
