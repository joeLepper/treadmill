...
@pytest.mark.asyncio
async def test_handle_scheduled_tick_routes_close_sweep_to_deterministic_path() -> None:
    """A scheduled tick for ``wf-escalation-close-sweep`` runs the
    deterministic sweep — it does NOT call the synthetic-task dispatch
    helper and therefore does NOT look up a ``WorkflowVersion``.
    ...
    mock_schedule.workflow_id = ESCALATION_CLOSE_SWEEP_WORKFLOW_ID

    session = AsyncMock()
    session.get = AsyncMock(return_value=mock_schedule)
    # If the deterministic intercept fires, session.execute MUST NOT be
    # called for the WorkflowVersion lookup.
    session.execute = AsyncMock()

    typed = ScheduledTick(
        schedule_id=schedule_id,
        workflow_id=ESCALATION_CLOSE_SWEEP_WORKFLOW_ID,
        rendered_payload={},
    )

    with patch.object(
        sweep_mod, "run_escalation_close_sweep", new=AsyncMock(return_value=0),
    ) as mocked_sweep:
    ...
    # synthetic-task path's WorkflowVersion SELECT.
    session.execute.assert_not_awaited()


# ── Task missing from DB is a no-op ──────────────────────────────────────────


@pytest.mark.asyncio
async def test_missing_task_skips_emission() -> None:
    """If the task row is gone (admin delete, race), the emit short-
    circuits with a warning log — no exception, no half-written event."""
    task_id = uuid.uuid4()
    opened_at = _opened_at(minutes_ago=4)
    ...
    session.get = AsyncMock(return_value=None)  # task disappeared
    ...
    # The sweep counted it as a successful close attempt (no exception),
    # but no event landed because the task was gone.
    assert closed == 1
    dispatcher.persist_and_publish.assert_not_awaited()
