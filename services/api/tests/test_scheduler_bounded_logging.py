"""Tests for the scheduler's bounded logging helpers (ADR-0035).

Mirrors the autoscaler/deploy-watcher subprocess_logging tests — the two
modules share a shape but live in separate packages (services/api vs.
tools/local-adapter). The properties exercised here are the load-bearing
ones for the scheduler subprocess:

  - ``configure_rotating_logging`` actually rotates and creates the parent
    dir, so ``scheduler.log`` cannot grow without bound and the parent
    spawn site can pass a path under ``.treadmill-local/`` without
    pre-creating the directory.
  - ``RateLimitedErrorLogger`` logs the first occurrence of an error in
    full and then collapses repeats into bounded summaries — so a wedged
    DB or expired SNS credential no longer dumps a fresh traceback every
    poll iteration (the 463-traceback incident).
"""
...
from treadmill_api.scheduler.bounded_logging import (
...
def test_configure_rotating_logging_rotates_when_max_bytes_exceeded(
    ...
    log_file = tmp_path / "logs" / "scheduler.log"  # parent dir does not exist
    configure_rotating_logging(log_file, max_bytes=500, backups=3)

    logger = logging.getLogger("treadmill.test.scheduler.rotation")
    for i in range(50):
    ...
    backups = list(log_file.parent.glob("scheduler.log.*"))
...
def test_configure_rotating_logging_creates_parent_dir(tmp_path: Path) -> None:
    ...
    logging.getLogger("treadmill.test.scheduler.mkdir").info("hello")
    for h in logging.getLogger().handlers:
        h.flush()
...
def test_configure_rotating_logging_replaces_existing_handlers(
    ...
    ``logger.info`` (or a stray ``basicConfig``) before configure runs.
    Configure must swap it out so we don't double-write into the parent's
    redirected stream (which the parent now sets to DEVNULL anyway)."""
    root = logging.getLogger()
...
def test_rate_limited_logger_logs_first_in_full_then_summarizes() -> None:
    ...
    persistent error storm (e.g. the 463-traceback ConnectionRefusedError
    burst observed in the dependency outage)."""
    logger, handler = _named_logger("treadmill.test.scheduler.ratelimit.basic")
    rl = RateLimitedErrorLogger(logger, summary_every=50)

    exc = ConnectionRefusedError("postgres unreachable: 127.0.0.1:15432")
    for _ in range(200):
        rl.log(exc, "scheduler: tick failed")
...
def test_rate_limited_logger_reset_re_arms_fresh_traceback() -> None:
    """After ``reset()`` (called on a successful tick), the next failure —
    even one identical to the earlier burst — must log a fresh traceback.
    Without this the operator never sees a stack trace for a second
    incident of the same error type."""
    logger, handler = _named_logger("treadmill.test.scheduler.ratelimit.reset")
    rl = RateLimitedErrorLogger(logger, summary_every=50)
...
def test_rate_limited_logger_new_signature_re_arms_traceback() -> None:
    ...
    logger, handler = _named_logger("treadmill.test.scheduler.ratelimit.newsig")
    rl = RateLimitedErrorLogger(logger, summary_every=50)
...
def test_rate_limited_logger_summary_cadence_respects_summary_every() -> None:
    ...
    15th repeat — three summaries across 15 total repeats."""
    logger, handler = _named_logger("treadmill.test.scheduler.ratelimit.cadence")
...
def test_rate_limited_logger_rejects_zero_summary_every() -> None:
    logger, _ = _named_logger("treadmill.test.scheduler.ratelimit.bad")
