"""Unit tests for the validator-gold review-queue router (ADR-0070 substep 3 task 2).

Exercises route handlers with a stub async session — no live database. Mirrors
``test_routers_review_architect_gold.py`` in style: hermetic, fast, pins the HTTP
...
Coverage:
  ...
  * Auto-discovery contract: ``review_validator_gold`` appears in
    ``review_pkg.MOUNTED_MODULES`` and the review aggregator mounts the
    validator-gold paths.
"""
...
from treadmill_api.routers.review import review_validator_gold as vg_mod
...
def _row(
    *,
    ...
    return SimpleNamespace(
        ...
        script_excerpt="Check: test_example passed",
        artifact_excerpt="stdout: PASSED",
        llm_label=llm_label,
        llm_confidence=llm_confidence,
        llm_rationale="The verdict appears correct.",
...
def test_get_next_returns_rows_in_confidence_order() -> None:
    ...
    with TestClient(app) as client:
        resp = client.get("/api/v1/review/validator-gold/next", params={"limit": 10})
...
def test_get_next_empty_queue_returns_empty_list() -> None:
    ...
    with TestClient(app) as client:
        resp = client.get("/api/v1/review/validator-gold/next")
...
def test_post_label_round_trip() -> None:
    ...
    with TestClient(app) as client:
        resp = client.post(
            f"/api/v1/review/validator-gold/{row.id}/label",
            json={
    ...
    assert body["label_verdict"] == "correct-verdict"
...
def test_post_label_with_override_reason() -> None:
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert body["label_verdict"] == "wrong-verdict"
...
def test_post_label_404_when_row_absent() -> None:
    ...
    with TestClient(app) as client:
        resp = client.post(
            f"/api/v1/review/validator-gold/{missing_id}/label",
            json={"label": "correct-verdict", "labeled_by": "operator"},
        )
...
def test_post_label_409_when_already_labeled() -> None:
    ...
    row = _row(label_verdict="correct-verdict")
    app = _build_app(_StubSession(rows=[row]))
    with TestClient(app) as client:
        resp = client.post(
            f"/api/v1/review/validator-gold/{row.id}/label",
            json={"label": "wrong-verdict", "labeled_by": "operator"},
        )
...
def test_post_label_422_when_label_not_in_closed_set() -> None:
    """``label`` outside the three-value closed set → 422 from Pydantic."""
    row = _row()
    app = _build_app(_StubSession(rows=[row]))
    with TestClient(app) as client:
        resp = client.post(
...
def test_post_label_422_when_labeled_by_empty() -> None:
    ...
    with TestClient(app) as client:
        resp = client.post(
            f"/api/v1/review/validator-gold/{row.id}/label",
            json={"label": "correct-verdict", "labeled_by": ""},
        )
    assert resp.status_code == 422
...
def test_get_row_returns_full_shape() -> None:
    ...
    app = _build_app(_StubSession(rows=[row]))
    with TestClient(app) as client:
        resp = client.get(f"/api/v1/review/validator-gold/{row.id}")
    assert resp.status_code == 200
    body = resp.json()
    assert body["id"] == str(row.id)
...
def test_get_row_404_when_absent() -> None:
    ...
    app = _build_app(_StubSession(rows=[]))
    with TestClient(app) as client:
        resp = client.get(f"/api/v1/review/validator-gold/{missing_id}")
    assert resp.status_code == 404
...
def test_get_stats_empty_queue() -> None:
    ...
    app = _build_app(_StubSession(rows=[]))
    with TestClient(app) as client:
        resp = client.get("/api/v1/review/validator-gold/stats")
    assert resp.status_code == 200
    body = resp.json()
    assert body["total"] == 0
...
def test_get_stats_label_accuracy_correct_fraction() -> None:
    ...
    matched_rows = [
        _row(llm_label="correct-verdict", label_verdict="correct-verdict") for _ in range(3)
    ]
    mismatched_rows = [
        _row(llm_label="correct-verdict", label_verdict="wrong-verdict") for _ in range(2)
    ]
...
def test_get_stats_with_unlabeled_rows() -> None:
    ...
    labeled = [_row(llm_label="correct-verdict", label_verdict="correct-verdict") for _ in range(2)]
...
def test_review_validator_gold_is_auto_discovered() -> None:
    """Auto-discovery picks up ``review_validator_gold.py`` without editing ``__init__.py``."""
    assert "review_validator_gold" in review_pkg.MOUNTED_MODULES
    paths = {getattr(route, "path", None) for route in review_pkg.router.routes}
    assert "/api/v1/review/validator-gold/next" in paths
    assert "/api/v1/review/validator-gold/stats" in paths
...
def test_init_does_not_enumerate_sibling_module_names() -> None:
    ...
    assert "review_validator_gold" not in source, (
        "routers/review/__init__.py mentions 'review_validator_gold' by name — "
        "auto-discovery should not enumerate siblings"
    )


def test_review_validator_gold_module_exposes_router_attribute() -> None:
    """Sibling contract: the module exports a top-level ``router``."""
    assert isinstance(vg_mod.router, APIRouter)
...
def test_discovery_picks_up_a_freshly_added_sibling() -> None:
    ...
    sibling_path = pkg_dir / "_test_synthetic_review_sibling_vg.py"
    sibling_path.write_text(
        "from fastapi import APIRouter\n"
        "router = APIRouter()\n"
        "@router.get('/_synthetic-review-probe-vg')\n"
        "async def _probe() -> dict:\n"
        "    return {'ok': True}\n"
    )
    try:
        sys.modules.pop(
            "treadmill_api.routers.review._test_synthetic_review_sibling_vg",
            None,
        )
        ...
        assert "_test_synthetic_review_sibling_vg" in mounted
        app = FastAPI()
        app.include_router(fresh_aggregator)
        paths = {getattr(route, "path", None) for route in app.routes}
        assert "/api/v1/review/_synthetic-review-probe-vg" in paths
    finally:
