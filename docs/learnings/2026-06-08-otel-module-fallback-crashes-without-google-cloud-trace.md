...
trigger: surprise
status: captured
related: DED Cloud Run dev deploy, ramjac_events 0.1.x
---

# Learning: ramjac_events OTel module crash-loops when OTEL_EXPORTER_OTLP_ENDPOINT is unset and google-cloud-trace is not installed
...
## Observation
`ramjac_events/otel.py` (≥0.2.0) has two branches:
...
The DED Docker image includes `ramjac-events` but not `opentelemetry-exporter-gcp-trace` (which provides `google.cloud.trace_exporter`). With no OTEL collector endpoint configured in the dev Pulumi stack, the fallback path triggered and crashed every container.
...
- Long-term: patch `ramjac_events/otel.py` to catch `ImportError` on the CloudTraceExporter branch and log a warning.
