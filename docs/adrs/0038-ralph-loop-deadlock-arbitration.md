...
**Deadlock signal (twice-widened 2026-05-15).** A `wf-feedback` step completes with `decision` in `{'responded-without-change', 'fail'}` while the underlying task's most-recent gate-bearing workflow returned a blocking verdict — *either* `wf-review.decision='changes_requested'` *or* `wf-validate.decision='fail'`. The same load-bearing-gate-still-blocks-merge-while-feedback-can't-make-progress shape, regardless of whether the feedback role declined to act (`responded-without-change`) or tried and gave up (`fail`).

The original predicate was review-centric (the first observed instance, on PR #83, was reviewer-driven). The validate axis was added the same day after task `c5438ed1` (Plan B observability) hit functionally the same deadlock with the *validator* saying `fail` while the reviewer said `approved` and feedback responded-without-change. The `fail` decision was added a few hours later, after observing tasks `472e3ddc`, `2a3eaadb`, `b25b3f5d` dead-end at `wf-feedback.decision='fail'` with no auto-dispatch — wf-feedback failing the same way as `responded-without-change` keeps the task stalled and warrants the same arbitration. Both gates are load-bearing per ADR-0029/ADR-0036; both can produce a deadlock the architect should arbitrate. **Limitation:** today's override mechanism (the `review.override` event flipping `review_decision='approved'` in the mergeability VIEW) covers only the review axis. For a validate-fail-driven deadlock the architect's `accept-as-is` will not unblock merge — the architect should use `amend` (route to wf-plan) to author the missing pieces. A `validate.override` mirror is a follow-up.
