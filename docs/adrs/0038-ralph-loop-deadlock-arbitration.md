...
**Deadlock signal (widened 2026-05-15).** A `wf-feedback` step completes with `decision='responded-without-change'` while the underlying task's most-recent gate-bearing workflow returned a blocking verdict — *either* `wf-review.decision='changes_requested'` *or* `wf-validate.decision='fail'`. Either is the same shape: a load-bearing gate still blocks merge, but feedback declined to author changes.

The original predicate was review-centric (the first observed instance, on PR #83, was reviewer-driven). The validate axis was added the same day after task `c5438ed1` (Plan B observability) hit functionally the same deadlock with the *validator* saying `fail` while the reviewer said `approved` and feedback responded-without-change. Both gates are load-bearing per ADR-0029/ADR-0036; both can produce a deadlock the architect should arbitrate. **Limitation:** today's override mechanism (the `review.override` event flipping `review_decision='approved'` in the mergeability VIEW) covers only the review axis. For a validate-fail-driven deadlock the architect's `accept-as-is` will not unblock merge — the architect should use `amend` (route to wf-plan) to author the missing pieces. A `validate.override` mirror is a follow-up.
