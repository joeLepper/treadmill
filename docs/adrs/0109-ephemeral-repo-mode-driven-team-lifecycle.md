...
- **Status:** accepted (2026-09-13; cross-model panel review — findings folded — + sibling co-sign by Ernie across two rounds, verified in-text). AMENDED BY ADR-0112 (2026-09-14): primary team teardown is EVENT-DRIVEN by the exec-in-charge on the `plan.handoff_pr_opened` done-signal (an external actor — the coordinator cannot tear down its own team without self-killing); the `team reconcile` timer is demoted from the primary teardown loop to a low-frequency BACKSTOP (+ standup/crash-revival). DRAIN-GUARD AMENDMENT (2026-09-13, panel review of the v1 plan docs/plans/2026-09-13-team-lifecycle-and-feature-branch-v1.md): the drain-guard blocks on TEAM-ACTIVE work only. `escalated` tasks and the `branch → main` handoff PR are PARKED-ON-HUMAN — they do NOT block teardown (blocking on a stuck escalation forever would pin the team and defeat the resource goal), and are tracked so the team re-stands-up on the human's action. The Decision body's blocking set is corrected in-cycle to exclude `escalated`. Parking is safe only if (a) the escalated task's work is DURABLE at escalation (committed/pushed, not uncommitted in a live worker) and (b) re-standup FIRES on the human's escalation-response, not only on `plan.submitted` — both are pinned as drain-guard foils in the v1 plan.
- **Date:** 2026-09-13
...
- **Drain-guard (DEFINED here, not deferred — it is the load-bearing safety
  ...
  is in a non-terminal TEAM-ACTIVE state. The blocking set is
  `registered | assigned | executing | pr_open | rework-pending` (NOT `escalated` —
  that is parked-on-human, see the amendment), AND — critically — **post-merge
  observation**: a task whose PR merged but whose deploy /
