...
const PREFIX = 'GERALD_INBOUND_V1\n';
...
function validateMessage(value) {
  if (!object(value) || Object.keys(value).some(k => !['dedupKey', 'from', 'text', 'model'].includes(k)) ||
    ...
    throw new Error('Expected {dedupKey, from, text[, model]}: nonempty strings (limits 4KiB/256B/64KiB)');
  }
  // model is optional; when present it becomes a CLI arg (argv, no shell), but keep
  // it to safe model-id characters and a short length as defense in depth.
  if (value.model !== undefined && (!nonempty(value.model) || !/^[A-Za-z0-9._-]{1,64}$/.test(value.model))) {
    throw new Error('model, when present, must match [A-Za-z0-9._-]{1,64}');
  }
}
...
async function queue({ threadId, message, model }) {
  // No shell interpolation; never change/restart the app-server daemon here.
  // A per-message model override (--model) selects a different OpenCode Go model
  // for this turn (ADR-0104 per-message model selection).
...
export function findRollout(threadId, sessionsDir = join(process.env.CODEX_HOME || join(homedir(), '.codex'), 'sessions')) {
  const found = [];
  function walk(dir) {
...
export class Inbound extends EventEmitter {
  ...
  async deliver(message) {
    ...
    try {
      await this.enqueue({ threadId: this.threadId, message: prepared.message, model: prepared.record.model });
      this._locked(() => {
