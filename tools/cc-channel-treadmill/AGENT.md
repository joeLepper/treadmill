...
- **Transactional relay delivery (task ecd6d6eb — the 2026-06-12 drain-without-delivery race)**: on a `--resume` launch the server drained the relay inbox to zero files while the resumed session never saw the content (worker-3's countermand, ~3h latency). Two compounding defects, both fixed in the new `relay-inbox.ts` (`RelayInbox`): the old `processRelayFile` was **unlink-before-notify** (destructive before delivery — a failed/landed-nowhere notification lost the message irrecoverably), and the startup drain ran right after `mcp.connect()`, which only opens the stdio transport — notifications written before the client's MCP `initialized` handshake land nowhere. Now: files are unlinked ONLY after the notification resolves (the #310 peek→deliver→commit lesson one layer up); the startup drain is **gated on attach** (`oninitialized` callback, with `getClientVersion()` as the already-initialized probe); a 60s **resweep** retries files retained by failed sends (fs.watch never re-fires for an existing file); and an in-flight set replaces the old unlink-first idempotency against fs.watch's duplicate events. Semantics move from at-most-once to AT-LEAST-ONCE (crash between notify and unlink redelivers on restart — correct for relay briefs: duplicates are visible and task-tagged, silent loss cost 3h live). Tests `relay-inbox.test.ts` (9: a verbatim-miniature CONTROL reproducing the loss, the retention+resweep fix, attach gating, subfolder meta, idempotent attach, watch-dup suppression, hygiene) — real files, scripted notifier seam.
