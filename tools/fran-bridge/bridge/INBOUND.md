...
Live queue-to-envelope fidelity is RUN and PASS (round-trips verified it).
Daemon-restart survival is characterized: an enqueued turn does NOT auto-execute
after an app-server restart — Codex pauses the recovered queue, and paused is not
absent, so a restart is never proof the turn is gone. Recovery requires explicit
handling (resume the pending entry where supported, otherwise an authorized replay
that accepts duplicate-execution risk); the paused-queue recovery piece is still
in progress (see README follow-up 3). A Codex version or rollout-format change must
revalidate this adapter before using its commits to acknowledge the bus. This
module does not provide bus FIFO scheduling; Alan's caller owns ordered delivery
and retry policy.
