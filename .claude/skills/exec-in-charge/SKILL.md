...
**A co-sign is an ADVERSARIAL review, not a rubber stamp — invoke `/adversarial-review`.** The co-signing sibling runs the `adversarial-review` skill against the PR: ground-truth the diff (not the description), name the invariant, construct a foil and run it, drive the error path, and prove each claimed guard can fail before trusting its pass. A co-sign asserts *"I verified this myself"* — if the reviewer did not run the foil, they say "approving on the author's evidence, not my own" and that stands on the record; it is not a full co-sign. A co-sign that only re-read the description is worthless — it launders the author's confidence into apparent consensus.

**For load-bearing work, get an INDEPENDENT adversarial pass from a SECOND reviewer — one who did not co-sign, on a different team where one exists.** *Load-bearing* is NOT self-assessed by feel — a fuzzy trigger the merging driver scopes to zero under time pressure is the hole. It is the same blast-radius set that earns `auto_merge:false`: a shared schema / migration / contract / CDK / infra change, anything security-sensitive, or a change to a high-fan-in / core module — **and when in doubt, treat it as load-bearing.** A type or shape change to a shared model is load-bearing by this definition; that is the F4 class this gate exists to catch, and a driver must not reclassify it as "small." The co-sign and the independent adversarial review are two different layers, and the second catches what the first misses. This is not belt-and-suspenders theatre: it has caught real silent-failure bugs a clean co-sign passed over — a schema-bump PR whose co-sign verified the model, while an independent adversarial reviewer found a missed consumer that crashed on the new type and was swallowed by a broad `except` into a mislabeled "call failed." Dispatch the second review with the load-bearing claim to attack (not a "please review"), and hold the merge until BOTH the co-sign and the independent pass are clean. Ask for a foil, not an opinion.

**A co-sign covers only the commit it SAW.** If new commits land after a co-sign — a review finding fixed, a rebase, anything — the prior co-sign is stale. Do not merge on it. Ask the co-signer to re-verify the delta (name the exact HEAD and the commits added), and hold the merge until they re-confirm on the current HEAD. A co-sign on `abc123` is not a co-sign on `def456`.
