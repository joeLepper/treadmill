...
3. **Get an adversarial review before the ADR is treated as settled** — run the cross-model review panel on the ADR file, plus a same-family sibling review. An ADR is high-stakes: `python3 ~/treadmill/tools/model-review-panel/panel.py review --artifact <adr_path> --author-family claude --min-cross-model 2` convenes multiple non-Anthropic voices (open-weight families via the gateway + GPT) alongside Claude in one command — the TWO-independent-cross-model-passes requirement, replacing BOTH the Tapestry evaluator agents and the retired gerald-bridge/fran-bridge relay (ADR-0105, ADR-0107). The panel fails closed (needs a cross-family quorum) and never counts a weak/empty reviewer as approval; re-run or record who actually reviewed. Then relay to a sibling (`send <sibling>`) for a context-rich same-family pass. Coverage is ENFORCED: `--author-family claude --min-cross-model 2` makes the panel fail closed (non-zero, `reduced-coverage`) when fewer than two genuinely non-Anthropic families vote — e.g. during a Go cap when only GPT survives (Claude is the author's own family). On reduced coverage, do not treat the ADR as cross-model-reviewed: wait for the open-weight leg (cap reset / top up) or get a second non-Anthropic voice. The author is the worst-placed to see the decision's own blind spots. The review must attack, not summarize: is the decision **falsifiable** and is it exactly ONE decision per ADR? Do the Alternatives lead with the incumbent — the do-nothing / platform-native option — rejected for a real reason, not a strawman? Are the Consequences honest about the bad and the risks? Address blocking findings before you rely on the ADR to drive a plan.
