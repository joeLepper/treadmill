...
- ADR-0020 Wave 1 — Per-step token usage now flows to the API on `step.completed` in addition to the OTel counters. `CodeAuthorResult` carries the parsed `token_usage` dict + `model` from the Claude Code JSON envelope; `runner._execute` returns `(StepOutput, StepTokenUsage | None)` and the publisher folds the typed sub-model into `step.completed.token_usage` (a distinct field — NOT in `StepOutput.metadata` because token usage is step-execution telemetry, not part of the polymorphic role/workflow envelope). `None` when the step made no LLM call (wf-validate, dry-run). The OTel `record_token_usage` call is unchanged.
